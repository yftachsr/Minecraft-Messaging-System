...
@OnlyIn(Dist.CLIENT)
public class ClearEditBox extends AbstractWidget implements Widget, GuiEventListener {
   ...
   public ClearEditBox(Font pFont, int pX, int pY, int pWidth, int pHeight, Component pMessage) {
      this(pFont, pX, pY, pWidth, pHeight, (ClearEditBox)null, pMessage);
   }

   public ClearEditBox(Font pFont, int pX, int pY, int pWidth, int pHeight, @Nullable ClearEditBox p_94111_, Component pMessage) {
   ...
   public void renderButton(PoseStack pPoseStack, int pMouseX, int pMouseY, float pPartialTick) {
      if (this.isVisible()) {
         if (this.isBordered()) {
            fill(pPoseStack, this.x - 1, this.y - 1, this.x + this.width + 1, this.y + this.height + 1, 0);
            fill(pPoseStack, this.x, this.y, this.x + this.width, this.y + this.height, 0);
         }
   ...
   public int getScreenX(int pCharNum) {
      return pCharNum > this.value.length() ? this.x : this.x + this.font.width(this.value.substring(0, pCharNum));
   }
   ...
   public void updateNarration(NarrationElementOutput pNarrationElementOutput) {
      pNarrationElementOutput.add(NarratedElementType.TITLE, Component.translatable("narration.clear_edit_box", this.getValue()));
   }
}